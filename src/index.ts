...
server.registerTool(
    ...
    async ({boardNameFilter, projectFilter}) => {
        // 1. Find board
        ...
        const cards = await trelloClient.getListCards(list.id, 'id,name');
...
server.registerTool(
    "get-next-actions-prompt",
    {
        ...
        inputSchema: {
            boardId: z.string(), cardId: z.string()
        }
    },
    async ({boardId, cardId}) => {
        ...
        const fieldItems = await trelloClient.getCardCustomFieldItems(cardId);
